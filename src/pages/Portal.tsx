...
const Header = ({ activeTab, setActiveTab }: HeaderProps) => (
  <header className="flex flex-col md:flex-row justify-between items-center bg-white px-8 py-4 rounded-2xl shadow-sm border border-slate-200 gap-4">
    ...
    <nav className="flex gap-4 sm:gap-8 items-center">
      ...
      <Link
        to="/deals"
        className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-sm"
      >
        <Plus size={14} /> Создание сделок
      </Link>
        ...
        {activeDeals.length > 0 ? activeDeals.map((lot: any, idx: number) => (
          <div key={idx} className="bg-white rounded-3xl border border-slate-200 p-8 flex flex-col gap-6 shadow-sm hover:shadow-xl hover:border-slate-300 transition-all group overflow-hidden relative">
            ...
            <div className="flex justify-between items-start z-10">
              <span className="px-3 py-1 bg-slate-100 rounded-lg text-[10px] font-black uppercase text-slate-500 tracking-widest">{cleanText(lot.type)}</span>
            ...
            <div className="z-10">
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 italic">{cleanText(lot.city)}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-y border-slate-50 py-4 z-10">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-slate-900 uppercase tracking-tighter">{cleanText(lot.status)}</p>
              </div>
            </div>
...
const getInvestorsTotal = (deal: Deal) => (deal.investors || []).reduce((sum, investor) => sum + parseMoney(investor.amount), 0);

const getDealCapital = (deal: Deal) => {
...
const getAnnualRent = (deal: Deal) => {
  return parseMoney(deal.areaSqm) * parseMoney(deal.rentRatePerSqm) * 12;
};

const hasRentModel = (deal: Deal) => parseMoney(deal.areaSqm) > 0 && parseMoney(deal.rentRatePerSqm) > 0;

const getNetAnnualFlow = (deal: Deal) => {
  ...
  return (parseMoney(deal.invested) || getDealCapital(deal)) * (parsePercent(deal.targetIrr) / 100) - parseMoney(deal.utilities) * 12;
};
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <AssetAllocation />
        <PaymentsCalendar />
        <ProjectsTable forecastConfig={forecastConfig} onSelectProject={setSelectedProjectId} currencyState={currencyState} />
        <NewsFeed />
      </motion.div>
    </AnimatePresence>
  );
...
const SummaryCard = ({ forecastConfig, setForecastConfig, currencyState }: {
  ...
  return (
  <section className="col-span-12 lg:col-span-4 bg-slate-900 text-white rounded-3xl p-5 flex flex-col justify-between shadow-lg relative overflow-hidden group">
    ...
    <div className="z-10">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end gap-3 min-h-[68px]">
        <p className="text-[38px] lg:text-[42px] font-light tabular-nums leading-none min-w-[260px] whitespace-nowrap">{formatCurrency(totalInvested, currency, rates)}</p>
        <div className="flex rounded-xl bg-white/5 border border-white/10 p-1 w-[174px] shrink-0">
          {(['RUB', 'USD', 'EUR'] as CurrencyCode[]).map(item => (
            <button
              key={item}
              onClick={() => setCurrency(item)}
              className={`w-14 px-0 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-colors ${currency === item ? 'bg-white text-slate-900' : 'text-white/50 hover:text-white'}`}
            >
              {item}
            </button>
          ))}
      ...
      <p className="text-emerald-400 text-xs mt-2 font-medium flex items-center gap-1">
        <ArrowUpRight size={14} /> {formatCurrency(projectedIncome, currency, rates)} ({projectedReturn.toFixed(1)}%) <span className="opacity-60 font-normal">прогноз {getForecastLabel(forecastConfig)}</span>
      </p>
      <p className="text-[9px] text-white/35 mt-2 font-bold uppercase tracking-widest min-h-[14px]">
        ЦБ: USD {rates.USD.toFixed(2)} ₽ · EUR {rates.EUR.toFixed(2)} ₽
      </p>
      <ForecastControls config={forecastConfig} onChange={setForecastConfig} />
    </div>
    <div className="grid grid-cols-2 gap-4 mt-4 border-t border-white/10 pt-4 z-10">
      <div>
        <p className="text-[10px] uppercase opacity-50 mb-1">Фактически выплачено</p>
        <p className="text-lg font-semibold tabular-nums">{formatCurrency(paidOut, currency, rates)}</p>
      </div>
      <div>
        <p className="text-[10px] uppercase opacity-50 mb-1 whitespace-nowrap">Прогноз дохода</p>
        <p className="text-lg font-semibold tabular-nums">{formatCurrency(projectedIncome, currency, rates)}</p>
      </div>
    </div>
  </section>
  );
...
const ProjectsTable = ({ forecastConfig, onSelectProject, currencyState }: {
  forecastConfig: ForecastConfig;
      ...
      <table className="w-full text-left border-collapse table-fixed min-w-[1040px]">
        <thead className="text-[10px] uppercase text-slate-400 font-bold bg-white sticky top-0 z-10">
          <tr className="border-b border-slate-50">
            <th className="p-6 w-[30%]">Название / Локация</th>
            <th className="p-4 text-center w-[13%]">Этап</th>
            <th className="p-4 text-right w-[11%]">Срок</th>
            <th className="p-4 text-right w-[14%]">Вложено</th>
            <th className="p-4 text-center text-slate-900 w-[12%]">Окупаемость</th>
            <th className="p-4 text-right text-slate-900 w-[13%]">Прогноз ₽</th>
            <th className="p-4 text-right text-slate-900 w-[7%]">Прогноз %</th>
          </tr>
        </thead>
          ...
          {portfolioDeals.length > 0 ? portfolioDeals.map((project: Deal) => {
            ...
            >
              <td className="p-6">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-slate-400 uppercase tracking-tight font-medium mt-1">
                    <MapPin size={10} /> {cleanText(project.type)} • {cleanText(project.city)}
                  </div>
                </div>
              </td>
              <td className="p-4 text-center">
                <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase ${STATUS_COLORS[cleanText(String(project.status))] || 'bg-slate-100 text-slate-700'}`}>
                  {cleanText(String(project.status))}
                </span>
              </td>
              ...
              <td className="p-4 text-right font-medium text-slate-700 whitespace-nowrap">
                {formatCurrency(capital, currency, rates)}
              </td>
              <td className="p-4 text-center whitespace-nowrap">
                <div className="flex items-center justify-center">
                  <div className="flex flex-col text-center w-[96px]">
                    <span className={`text-sm font-bold tabular-nums ${paybackYears || parsePercent(project.targetIrr) > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
                      {paybackYears ? `${paybackYears.toFixed(2)} лет` : parsePercent(project.targetIrr) > 0 ? `${project.targetIrr}% год.` : '—'}
                    </span>
                  </div>
                </div>
              </td>
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase w-fit ${STATUS_COLORS[cleanText(String(deal.status))] || 'bg-slate-100 text-slate-700'}`}>
          {cleanText(String(deal.status))}
        </span>
      </div>
      ...
      <section className="bg-slate-900 text-white rounded-3xl p-8 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 xl:grid-cols-[1.2fr_1fr] gap-8">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-white/40 font-black mb-2">{cleanText(deal.type)} • {cleanText(deal.city)}</p>
            <h2 className="text-3xl lg:text-4xl font-black tracking-tight leading-tight">{deal.name}</h2>
            {(deal.description || deal.strategy) && (
              <p className="text-sm text-white/50 mt-3 max-w-2xl">{deal.description || deal.strategy}</p>
            )}
            <ForecastControls config={forecastConfig} onChange={setForecastConfig} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 content-start">
            <div className="border border-white/10 rounded-2xl p-4 bg-white/5">
              ...
              <p className="text-xl font-semibold tabular-nums">{formatRub(capital)}</p>
            </div>
            <div className="border border-white/10 rounded-2xl p-4 bg-white/5">
            ...
            <div className="border border-white/10 rounded-2xl p-4 bg-white/5">
              <p className="text-[9px] uppercase text-white/40 font-black mb-1">Окупаемость</p>
              <p className={`text-xl font-semibold tabular-nums ${paybackYears ? 'text-white' : 'text-rose-300'}`}>{paybackYears ? `${paybackYears.toFixed(2)} лет` : `${projectedPercent.toFixed(1)}%`}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <section className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
          <div className="mb-6">
            <h3 className="font-black text-slate-900 uppercase tracking-tight text-sm">История сделки</h3>
          </div>
...
const AssetAllocation = () => {
  ...
  return (
  <section className="col-span-12 md:col-span-6 lg:col-span-4 bg-white rounded-3xl border border-slate-200 shadow-sm p-5 flex flex-col">
    ...
    <div className="flex-1 flex items-center justify-around min-h-[210px]">
...
const paymentStyles: Record<PaymentStatus, { dot: string; card: string; text: string }> = {
  expected: { dot: 'bg-amber-400', card: 'bg-amber-50/70 border-amber-100', text: 'text-amber-700' },
  paid: { dot: 'bg-emerald-500', card: 'bg-emerald-50/70 border-emerald-100', text: 'text-emerald-700' },
  overdue: { dot: 'bg-rose-500', card: 'bg-rose-50/70 border-rose-100', text: 'text-rose-700' },
  closing: { dot: 'bg-slate-400', card: 'bg-slate-50 border-slate-100', text: 'text-slate-600' },
};
...
const PaymentsCalendar = () => {
  ...
  return (
    <section className="col-span-12 md:col-span-6 lg:col-span-4 bg-white rounded-3xl border border-slate-200 p-5 flex flex-col gap-3 shadow-sm relative overflow-visible">
      <div className="flex justify-between items-center">
        <button onClick={() => setExpanded(prev => !prev)} className="font-bold text-sm text-slate-900 flex items-center gap-2 hover:text-emerald-600 transition-colors">
          <Calendar size={16} className="text-slate-400" /> Календарь выплат
        </button>
      ...
      <div className="space-y-3">
        {upcoming.length ? upcoming.map(event => {
          const style = paymentStyles[event.status];
          return (
            <div key={event.id} title={`${event.dealName}: ${event.title}`} className={`flex justify-between items-center p-3 rounded-xl border ${style.card} transition-all`}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-lg bg-white flex flex-col items-center justify-center border border-white shadow-sm shrink-0">
                  <span className="text-[8px] font-bold text-slate-400 uppercase">{event.date.toLocaleDateString('ru-RU', { month: 'short' })}</span>
                  <span className="text-sm font-black text-slate-900">{event.date.getDate()}</span>
                </div>
                <div className="min-w-0">
                  <p className={`text-xs font-black ${style.text}`}>{event.title}</p>
                  <p className="text-[10px] text-slate-400 font-medium truncate">{event.dealName}</p>
                </div>
              </div>
              <span className={`text-xs font-black uppercase whitespace-nowrap ${style.text}`}>{event.amount > 0 ? formatSignedRub(event.amount) : event.title}</span>
            </div>
          );
        }) : (
          <div className="p-6 text-center text-xs text-slate-400 font-bold border border-dashed border-slate-200 rounded-xl">
      ...
      {expanded && (
        <div className="absolute inset-x-4 top-16 z-30 bg-white border border-slate-200 rounded-2xl shadow-2xl p-4">
          <div className="flex items-center justify-between gap-2 mb-3">
            <select
              value={viewMonth}
              onChange={event => setViewMonth(Number(event.target.value))}
              className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 focus:outline-none"
            >
              {Array.from({ length: 12 }, (_, month) => (
                <option key={month} value={month}>{new Date(viewYear, month, 1).toLocaleDateString('ru-RU', { month: 'long' })}</option>
              ))}
            </select>
            <select
              value={viewYear}
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-[9px] font-black uppercase text-slate-300 mb-2">
            ...
            {calendarDays.map(day => {
              ...
              return (
                <div
                  ...
                  className={`min-h-10 rounded-lg border p-1 text-[10px] font-bold ${isCurrentMonth ? 'bg-white border-slate-100 text-slate-700' : 'bg-slate-50 border-slate-50 text-slate-300'}`}
                >
                  <div>{day.getDate()}</div>
          ...
          <div className="flex flex-wrap gap-3 mt-3 text-[9px] font-bold uppercase text-slate-400">
